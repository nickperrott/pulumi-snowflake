...
class BaseDynamicProvider(ResourceProvider):
    ...
    def generate_sql_create_statement(self, name, inputs, environment=None):
    ...
    def create(self, inputs):
        ...
        if inputs.get("name") is None and inputs.get("resource_name") is None:
            raise Exception(
                "At least one of 'name' or 'resource_name' must be provided")

        validated_name = self._get_autogenerated_name(inputs)

        # Perform SQL command to create object
        environment = self._create_jinja_environment()
        sql_statement = self.generate_sql_create_statement(
            validated_name, inputs, environment)
        ...
        info(
            f"Creation of {self.resource_type} with name {validated_name} successful")

        return CreateResult(
            id_=validated_name,
            outs=self._generate_outputs(
                validated_name, inputs, provisional_outputs)
        )
    ...
    def diff(self, id, olds, news):
        ...
        ignoreFields = ["name", "resource_name", "full_name", "__provider"]
        ...
        info(
            f"Diff of {self.resource_type} with name {id} has changes: {len(changed_fields) > 0}")
    ...
    def delete(self, id, props):
        ...
        sql = self.generate_sql_drop_statement(
            id, props, self._create_jinja_environment())
        self._execute_sql(sql)
        info(
            f"Deletion of object {self.resource_type} with name {id} successful")

    def _get_autogenerated_name(self, inputs):
        """
    ...
    def _get_full_object_name(self, inputs, name):
        ...
        if inputs.get("database") or inputs.get("schema"):
            ...
            schema = Validation.enquote_identifier(schema)
    ...
    def _get_database_and_schema(self, inputs):
        ...
        if inputs.get("database") or inputs.get("schema"):
            ...
            database = inputs.get("database") if inputs.get(
    ...
    def _execute_sql(self, statement):
        debug(f"Executing SQL statements={statement}")
